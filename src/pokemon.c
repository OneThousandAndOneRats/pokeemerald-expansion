...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst)
{
    ...
    dst->types[0] = isDelta ? GetMonData(src, MON_DATA_DELTA_TYPE, NULL): gSpeciesInfo[dst->species].types[0];
    dst->types[1] = isDelta ? GetMonData(src, MON_DATA_TERA_TYPE, NULL): gSpeciesInfo[dst->species].types[1];
